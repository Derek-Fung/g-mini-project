    ...
    while True:
        ...
        # Orders menu #######################################################
            ...
            while True:
                i = main_input
                if user_input == 0:
                    print_menu(reports, 0) # Default menu  
                         
                if user_input == 1:
                    past_month_num_orders = []
                    for i in range(len(months_inv)):
                        sql = f'''SELECT COUNT(oid) as res FROM orders WHERE date > DATE("{months_inv[i][0]}") AND date < DATE("{months_inv[i][1]}")'''
                        cursor.execute(sql)
                    ...
                    x_pos = np.arange(len(months_name))
                    fig, ax = plt.subplots()
                    ...
                    past_month_result = []
                    for i in range(len(months_inv)):
                        sql = f'''SELECT SUM(order_total) as res FROM orders WHERE date > DATE("{months_inv[i][0]}") AND date < DATE("{months_inv[i][1]}")'''
                        cursor.execute(sql)
                    ...
                    print_menu(reports, user_input)
                    p(f"You are in user_input: {user_input}")
                    myinput = input(f'Spacebar to Pass{cc["E"]}: ')
                    if myinput in bypass_list:
                        print_menu(reports, 0) # Default menu  
                ...
                p(" ") # For nice looking
                user_input = input_option(reports, main_input)
                if user_input == 0:
                    main_input = 0
                    break
